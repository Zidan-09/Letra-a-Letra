...
export default function ChatPopup({
  ...
  return (
    <div
      className={local === "lobby" ? styles.overlayLobby : styles.overlayGame}
      onClick={handleClose}
    >
        ...
        <div className={styles.messages}>
          {messages.map((message, index) =>
            message.from === nickname ? (
              <p key={index} className={`${styles.m} ${styles.my}`}>
                <span className={styles.me}>{message.from}</span>:{" "}
                {message.message}
              </p>
            ) : (
              <p key={index} className={styles.m}>
                <span className={styles.others}>{message.from}</span>:{" "}
                {message.message}
              </p>
            )
          )}
