...
exports.GameSocket = {
    gameStarted(room_id) {
        ...
        if (room === serverResponses_1.ServerResponses.NotFound || !room.players || !room.board || !room.board.words)
            return;
        ...
        all.filter(Boolean).forEach(p => io.to(p.player_id).emit("game_started", data));
    },
        ...
        all.filter(Boolean).forEach(p => {
            if (p.player_id !== player_id)
                io.to(p.player_id).emit("movement", {
                    movement: movement,
                    player_id: player_id,
                    data: data.status === gameResponses_1.GameResponses.TrapTrigged ? data : data.status,
                    players: room.players,
                    turn: room.turn
                });
        });
    ...
    movementAll(room_id, player_id, movement, data) {
        const room = roomService_1.RoomService.getRoom(room_id);
        if (room === serverResponses_1.ServerResponses.NotFound || !room.players || !room.spectators)
            return;
        const players = room.players;
        const spectators = room.spectators;
        const all = [...players, ...spectators];
    ...
    passTurn(room_id) {
        ...
        const all = [...players, ...spectators];
        const io = (0, socket_1.getSocketInstance)();
        all.filter(Boolean).forEach(p => io.to(p.player_id).emit("pass_turn", { turn: room.turn }));
    },
    afkPlayer(player_id) {
        const io = (0, socket_1.getSocketInstance)();
    ...
    discardPower(room_id) {
        ...
        if (room === serverResponses_1.ServerResponses.NotFound)
            return;
        const all = [...room.players, ...room.spectators];
        all.filter(Boolean).forEach(p => io.to(p.player_id).emit("discard_power", room));
    }
};
