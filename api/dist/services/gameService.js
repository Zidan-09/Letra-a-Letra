...
exports.GameService = {
    startGame(room_id, theme, gamemode, allowedPowers) {
        ...
        if (!players)
            return serverResponses_1.ServerResponses.NOT_FOUND;
        if (players.length < 2)
            return gameResponses_1.GameResponses.NOT_ENOUGH_PLAYERS;
        ...
        (0, logger_1.createLog)(room_id, logEnum_1.LogEnum.GAME_STARTED);
        return gameResponses_1.GameResponses.GAME_STARTED;
    },
    ...
    moveGame(room_id, player_id, movement, powerIndex, x, y) {
        ...
        if (player.freeze.active &&
            ...
            player.decrementEffect();
            game.incrementTurn();
            return gameResponses_1.GameResponses.PLAYER_FROZEN;
        }
        player.decrementEffect();
        game.incrementTurn();
        ...
        switch (movement) {
            ...
            case movementsEnum_1.MovementsEnum.FREEZE:
                (0, logger_1.createLog)(room_id, `${player.nickname} ${logEnum_1.LogEnum.FREEZE} ${players
                    .filter(Boolean)
            ...
            case movementsEnum_1.MovementsEnum.BLIND:
                (0, logger_1.createLog)(room_id, `${player.nickname} ${logEnum_1.LogEnum.BLINDED} ${players
                    .filter(Boolean)
                    .find((p) => p.player_id !== player.player_id)?.nickname}`);
            ...
            default:
                (0, logger_1.createLog)(room_id, `${player.nickname} ${logEnum_1.LogEnum.INVALID_MOVEMENT}`);
